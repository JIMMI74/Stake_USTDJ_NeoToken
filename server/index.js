...
app.post("/register", async (req, res) => {
  ...
  if (!dbuser) {
    ...
    const insert = await usersAuth.insertOne({
      _id: email,
      encPass,
      salat,
      refreshToken,
    ...
    if (insert) {
      // prima di fare il redirect dovremmo salvere il token per riconoscerlonode
...
app.post("/login", async (req, res) => {
  ...
  if (encPass === encPassDaControllare) {
    // quindi le password corispondono
...
app.post("/user", authToken, (req, res) => {
  ...
  userRegister
    ...
    .then((user) => {
      console.log("tovato");
      userRegister
        .updateOne({ _id: email }, { $set: { wallet } })
        .then((e) => {
...
async function authToken(req, res, next) {
  ...
  console.log({ user });
  if (user === undefined) {
    // se il token non e piu valido posso dagli un mesaggio di sessione scaduta, redirect o rigeneraralo
    res.clearCookie("jwtoken");
